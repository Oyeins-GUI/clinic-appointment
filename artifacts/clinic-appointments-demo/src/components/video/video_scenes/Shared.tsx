...
import { CalendarDays, Check, Clock3, MapPin, Phone, ShieldCheck, Sparkles, UserRound } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={{ x: ['0%', '-4%', '0%'], y: ['0%', '3%', '0%'], rotate: [0, 18, 0] }}
...
export function Wordmark({ light = false, compact = false }: { light?: boolean; compact?: boolean }) {
  return (
    <div className="flex items-center gap-[.75vmin]" style={{ color: light ? '#fffaf3' : '#123837' }}>
      ...
      <span className={`${compact ? 'text-[2.1vmin]' : 'text-[2.6vmin]'} font-extrabold tracking-[-.07em]`}>
        NayaCare
      </span>
    </div>
  );
...
export function Kicker({ children, light = false }: { children: ReactNode; light?: boolean }) {
  return (
    <div className={`mono-label flex items-center gap-[1vmin] ${light ? 'text-[#f29d63]' : 'text-[#276b63]'}`}>
...
export function PhoneFrame({ children, className = '', dark = false }: { children: ReactNode; className?: string; dark?: boolean }) {
  return (
    <div className={`relative aspect-[.52] w-[26vmin] rounded-[4.8vmin] border-[.7vmin] ${dark ? 'border-[#0b2827] bg-[#0b2827]' : 'border-[#123837] bg-[#123837]'} p-[.75vmin] ${className}`}>
      <div className="absolute left-1/2 top-[.65vmin] z-20 h-[1.7vmin] w-[8vmin] -translate-x-1/2 rounded-full bg-[#0b2827]" />
      <div className="relative h-full overflow-hidden rounded-[3.7vmin] bg-[#f4efe8]">{children}</div>
    </div>
  );
}

export function AppTop({ title = 'NayaCare', compact = false }: { title?: string; compact?: boolean }) {
  return (
    <div className="flex items-center justify-between border-b border-[#123837]/10 px-[2.4vmin] py-[2vmin]">
      <div className="flex items-center gap-[.8vmin]">
        ...
        <span className={`${compact ? 'text-[1.35vmin]' : 'text-[1.6vmin]'} font-extrabold tracking-[-.06em] text-[#123837]`}>{title}</span>
      </div>
...
export function Chip({ children, active = false, orange = false }: { children: ReactNode; active?: boolean; orange?: boolean }) {
  return (
    <div className={`rounded-full px-[1.5vmin] py-[.85vmin] text-[1.25vmin] font-bold ${active ? 'bg-[#276b63] text-[#fffaf3]' : orange ? 'bg-[#f29d63]/25 text-[#9b4b31]' : 'bg-[#dcece5] text-[#276b63]'}`}>
      {children}
    </div>
  );
}

export function TinyIcon({ kind }: { kind: 'calendar' | 'clock' | 'pin' | 'phone' | 'user' | 'shield' }) {
  const Icon = { calendar: CalendarDays, clock: Clock3, pin: MapPin, phone: Phone, user: UserRound, shield: ShieldCheck }[kind];
  return <Icon size="1.8vmin" strokeWidth={2.2} />;
}

export function DetailRow({ icon, label, value }: { icon: 'calendar' | 'clock' | 'pin' | 'phone' | 'user' | 'shield'; label: string; value: string }) {
  return (
    <div className="flex items-center gap-[1.5vmin]">
      ...
      <div>
        <div className="text-[1.05vmin] font-semibold text-[#5f756f]">{label}</div>
        <div className="text-[1.45vmin] font-bold text-[#123837]">{value}</div>
      </div>
    </div>
  );
}

export function FloatingStamp({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <motion.div
...
export function ProgressRail({ active = 1, total = 4 }: { active?: number; total?: number }) {
  return (
    <div className="flex items-center gap-[.7vmin]">
      {Array.from({ length: total }).map((_, index) => (
        <span key={index} className={`h-[.7vmin] rounded-full transition-all ${index < active ? 'w-[3vmin] bg-[#276b63]' : 'w-[.7vmin] bg-[#b4c9c0]'}`} />
      ))}
    </div>
  );
}
...
export function SparkLine() {
  return (
    <svg viewBox="0 0 180 42" className="h-[6vmin] w-[19vmin] overflow-visible">
      <path d="M2 35 C18 34, 22 14, 42 21 S64 42, 82 27 S101 7, 117 17 S140 36, 178 4" fill="none" stroke="#f29d63" strokeWidth="3.5" strokeLinecap="round" />
      <path d="M2 35 C18 34, 22 14, 42 21 S64 42, 82 27 S101 7, 117 17 S140 36, 178 4 L178 42 L2 42Z" fill="#f29d63" opacity=".14" />
    </svg>
  );
}

export function SceneFooter({ page, total = '09' }: { page: string; total?: string }) {
  return (
    <div className="absolute bottom-[4vmin] left-[6vmin] right-[6vmin] z-20 flex items-center justify-between">
      ...
      <div className="ml-[2vmin] flex items-center gap-[1vmin] text-[1.1vmin] font-bold tracking-[.15em] text-[#5f756f]">
        <span>{page}</span><span className="text-[#123837]/30">/</span><span>{total}</span>
      </div>
    </div>
  );
}