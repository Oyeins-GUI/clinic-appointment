...
export function Header() {
   ...
   const links = [
      { to: "/", label: "Book a visit" },
      { to: "/my-appointments", label: "My appointments" },
      { to: "/clinic", label: "Clinic details" },
   ];
   return (
      <header className="relative z-10 border-b border-border/70 bg-background/90 backdrop-blur-md">
         <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 lg:px-8">
            ...
            >
               {links.map((link) => (
                  <Link
                     ...
                     className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors hover:bg-muted ${location.pathname === link.to ? "bg-muted text-primary" : "text-muted-foreground"}`}
                  >
                     {link.label}
                  </Link>
               ))}
            ...
            <Link
               to="/my-appointments"
               className="hidden items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold text-foreground transition hover:border-primary md:flex"
            ...
            <Link
               to="/my-appointments"
               className="grid size-10 place-items-center rounded-full bg-muted text-primary md:hidden"
...
export function SectionKicker({ children }: { children: ReactNode }) {
   return (
      <p className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-primary">
...
export function ErrorNotice({
   ...
   return (
      <div
         className="rounded-2xl border border-accent/30 bg-accent/10 p-5"
      ...
      >
         ...
         {onRetry && (
            <button
               onClick={onRetry}
               className="mt-4 rounded-full bg-foreground px-4 py-2 text-sm font-bold text-background transition hover:opacity-85"
...
export function EmptyState({
   ...
   return (
      ...
      >
         <div className="mx-auto mb-4 grid size-12 place-items-center rounded-2xl bg-muted text-primary">
   ...
   return (
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-xs font-semibold text-muted-foreground">
         <span className="inline-flex items-center gap-2">
            <ShieldCheck className="size-4 text-primary" /> Your details stay
            private
         </span>
         <span className="inline-flex items-center gap-2">
            <Clock3 className="size-4 text-primary" /> Usually under a minute
         </span>
      </div>
   );
}
