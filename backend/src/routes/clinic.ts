...
router.get("/appointment-slots", async (req, res): Promise<void> => {
   ...
   if (parsed.data.date) {
      conditions.push(
         eq(appointmentSlotsTable.date, formatDate(parsed.data.date)),
      );
   }
   ...
   const slots = await db
      .select({
         ...
         status: sql<
            "available" | "booked"
         >`CASE WHEN ${appointmentSlotsTable.status} = 'available' THEN 'available' ELSE 'booked' END`,
      })
      .from(appointmentSlotsTable)
      .innerJoin(
         doctorsTable,
         eq(appointmentSlotsTable.doctorId, doctorsTable.id),
      )
      .where(conditions.length ? and(...conditions) : undefined)
      .orderBy(
         asc(appointmentSlotsTable.date),
         asc(appointmentSlotsTable.startTime),
      );
...
router.get("/clinic-summary", async (_req, res): Promise<void> => {
   ...
   const todayAvailable = availableSlots.filter(
